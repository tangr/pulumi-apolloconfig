...
provider_debug::
	(cd provider && go build -o $(WORKING_DIR)/bin/${PROVIDER} -tags debug -gcflags="all=-N -l" -ldflags "-X ${PROJECT}/${VERSION_PATH}=${VERSION}" $(PROJECT)/${PROVIDER_PATH}/cmd/$(PROVIDER))
...
# install_nodejs_sdk::
#     -npm unlink --prefix $(WORKING_DIR)/sdk/nodejs/bin
